...
*/
select id, NAME, warrantystarton
from components
where facilityid=1
...
*/
Select id, assetidentifier ,serialnumber, name
from components
...
*/
Select  components.installatedon
from components
...
from component_types
where
    lower(name) like '%con%' and  expectedlife is null ;
...
from components
where
    facilityid=1 and warrantystarton is null ;
...
from spaces
where
   floorid=1 and id not IN (4,9,19) ;
...
del facility 1
*/
select name, id, warrantydurationparts
from component_types
where
   facilityid=1 and warrantydurationparts is not null  ;
/* 30
Lista de los tipos de componentes que no tiene el coste de repuesto
del facility 1
*/
select name, id, replacementcost
from component_types
where
   facilityid=1 and replacementcost is null ;
